...
\fancypagestyle{firstpage}{
  ...
  \fancyhead[L]{%
    \begin{minipage}[c]{0.78\textwidth}
      ...
      {\fontsize{8pt}{10pt}\selectfont E-ISSN: 2797-1724, DOI: 00.000/XXXX.YYYY.II.NNN}\\[1pt]
      {\fontsize{8pt}{10pt}\selectfont Vol. 1 No. 1 2025 pp. 01--10}
    \end{minipage}%
  }
  \fancyhead[R]{%
...
$^{4}$ Study Program of Graphic Multimedia Engineering Technology, Department of Computer and Informatics Engineering, Politeknik Negeri Medan, Jl. Almamater No. 1 USU Campus, Medan 20155, Indonesia\par}
\vspace{3pt}
...
\begin{figure}[htbp]
  \centering
  \IfFileExists{Gambar/arsitektur_sistem_bw.png}{\includegraphics[width=0.75\textwidth]{arsitektur_sistem_bw.png}}{\fbox{POS System Architecture Diagram}}
...
\begin{itemize}
  \item Mobile Client Application: Built with Flutter (Dart programming language) following the Riverpod reactive state management architecture \cite{flutter2026, remi2023riverpod}. Local persistence is managed by Isar Database \cite{isar2024docs}, which stores product catalogs, categories, and receipts instantaneously. A background Sync Engine periodically monitors network connectivity; upon detecting an active internet connection, it dispatches unsynchronized local transaction records marked with \texttt{is\_synced = false} to the cloud database.
  \item Cloud Infrastructure (Supabase): Utilizes managed Supabase PostgreSQL as a centralized relational database equipped with Row Level Security (RLS) policies \cite{supabase2024docs} and a stored procedure (\textit{Remote Procedure Call}/RPC) named \texttt{create\_transaction\_v4} to execute atomic transaction records and inventory deductions.
...
The empirical dataset comprises authentic transactional records collected from the EatsTEDI SV UGM canteen spanning August 26, 2024, to June 21, 2026, comprising 314 daily records and over 94,000 transaction line items. A representative preview of the daily sales time-series dataset is presented in Table \ref{tab:sample_dataset}.

\begin{table}[htbp]
  \centering
  \caption{Sample Preview of the Daily Sales Time Series Dataset (EatsTEDI UGM)}
  \label{tab:sample_dataset}
  \fontsize{8pt}{10.5pt}\selectfont
  \begin{tabular}{crrrrrrc}
    \toprule
    \textbf{Date} & \textbf{Revenue (IDR)} & \textbf{Trx} & \textbf{Qty} & \textbf{DOW} & \textbf{Weekend} & \textbf{Holiday} & \textbf{Ramadan} \\
    \midrule
    2024-08-26 & 1,701,500 & 113 & 585 & 0 & 0 & 0 & 0 \\
    2024-08-27 & 1,746,500 & 245 & 611 & 1 & 0 & 0 & 0 \\
    2024-08-28 & 1,650,500 & 179 & 570 & 2 & 0 & 0 & 0 \\
    2024-08-29 & 1,814,500 & 219 & 646 & 3 & 0 & 0 & 0 \\
    2024-08-30 & 1,233,500 & 167 & 453 & 4 & 0 & 0 & 0 \\
    2024-09-02 & 1,393,500 & 176 & 489 & 0 & 0 & 0 & 0 \\
    \bottomrule
  \end{tabular}
\end{table}

Across the operational timeline, data preprocessing was systematically conducted through the following stages:
\begin{itemize}
  ...
  \item Normalization and Sequence Construction: Continuous numerical features were scaled into $[0, 1]$ via Min-Max Scaling \cite{pedregosa2011scikit}. Supervised learning samples were generated using a sliding window across 7 active trading days ($look\_back = 7$), resulting in an input tensor dimension of $7 \times 8$.
...
The layer configuration of the Hybrid LSTM model is summarized in Algorithm \ref{alg:lstm_model}, and the training hyperparameters are listed in Table \ref{tab:param_pelatihan}. Optimization was carried out using the Adam algorithm \cite{kingma2015adam}, complemented by dropout regularization \cite{srivastava2014dropout} to prevent model overfitting.
...
Forecasting accuracy was examined under a one-step-ahead protocol utilizing three complementary evaluation metrics \cite{hyndman2006, hodson2022}:
\begin{align}
...
\begin{figure}[htbp]
  \centering
  \IfFileExists{Gambar/prediksi_vs_aktual.png}{\includegraphics[width=0.75\textwidth]{prediksi_vs_aktual.png}}{\fbox{Actual vs Forecasted Revenue Plot}}
...
\begin{figure}[htbp]
  \centering
  \IfFileExists{Gambar/boxplot_multiseed.png}{\includegraphics[width=0.48\textwidth]{boxplot_multiseed.png}}{\fbox{Multi-Seed MAE Boxplot}}
...
\begin{figure}[htbp]
  \centering
  \IfFileExists{Gambar/smart_analytics.png}{\includegraphics[width=0.45\textwidth]{smart_analytics.png}}{\fbox{POS Smart Analytics Interface}}
...
Comprehensive black-box verification was executed across 12 core operational use cases covering both Owner and Cashier roles in compliance with software testing standards \cite{pressman2020, myers2011, iso2021}. Every test case completed successfully with a 100\% PASS outcome (Table \ref{tab:blackbox}).
...
E. M. Sipayung, C. Fiarni, and Wawan, ``Evaluation of Point of Sale Application Usage Using Technology Acceptance Model in MSMEs,'' \textit{Jurnal Nasional Teknik Elektro dan Teknologi Informasi (JNTETI)}, vol. 9, no. 1, pp. 18--24, 2020, doi: 10.22146/jnteti.v9i1.116.

\bibitem{siddik2020}
M. Siddik and S. Samsir, ``Design and Implementation of POS (Point of Sale) Information System for Cashiers Using Object-Oriented Programming Concepts,'' \textit{JOISIE (Journal of Information Systems and Informatics Engineering)}, vol. 4, no. 1, pp. 43--49, 2020, doi: 10.35145/joisie.v4i1.607.

\bibitem{flutter2026}
Google LLC, ``Flutter Documentation: Offline-first Architecture and Local Persistence,'' 2026. [Online]. Available: \url{https://docs.flutter.dev/}. [Accessed: 29-Jul-2026].

\bibitem{hyndman2021}
R. J. Hyndman and G. Athanasopoulos, \textit{Forecasting: Principles and Practice}, 3rd ed. Melbourne, Australia: OTexts, 2021.

\bibitem{box2015}
G. E. P. Box, G. M. Jenkins, G. C. Reinsel, and G. M. Ljung, \textit{Time Series Analysis: Forecasting and Control}, 5th ed. Hoboken, NJ, USA: John Wiley \& Sons, 2015, doi: 10.1002/9781118619193.
...
\bibitem{yadav2025}
P. Yadav and C. R. Barde, ``Retail Real-Time Sales Prediction System Using LSTM and XGBoost,'' \textit{International Journal of Advanced Research in Computer and Communication Engineering (IJARCCE)}, vol. 14, no. 4, pp. 1--9, 2025, doi: 10.17148/IJARCCE.2025.14401.

\bibitem{sembiring2025}
R. A. Sembiring and Y. Sary, ``Analysis and Implementation of Long Short-Term Memory (LSTM) for Inventory Demand in PT. Gunung Sari Indonesia,'' \textit{Jurnal Ilmiah Teknik Mesin, Elektro dan Komputer}, vol. 5, no. 1, pp. 112--124, 2025.

\bibitem{jannah2025}
U. M. Jannah, D. S. Hormansyah, and D. P. Hapsari, ``Analysis of Water Refill Sales Prediction Using LSTM and SARIMA Methods,'' \textit{Jurnal Teknologi Informasi dan Multimedia}, vol. 7, no. 2, pp. 88--97, 2025.

\bibitem{verdiyanto2025}
R. Verdiyanto, D. Hartanti, and E. Purwanto, ``Development of Point of Sales Application for Beverage Enterprise Daily Sales Prediction Using Random Forest Regression Algorithm,'' \textit{Infotek: Jurnal Informatika dan Teknologi}, vol. 8, no. 2, pp. 310--321, 2025, doi: 10.29408/jit.v8i2.25681.
...
\bibitem{myers2011}
G. J. Myers, C. Sandler, and T. Badgett, \textit{The Art of Software Testing}, 3rd ed. Hoboken, NJ, USA: John Wiley \& Sons, 2011, doi: 10.1002/9781119202486.
...
R. Rousselet, ``Riverpod: A Reactive Caching and Data-Binding Framework for Flutter,'' 2023. [Online]. Available: \url{https://riverpod.dev/}. [Accessed: 29-Jul-2026].

\bibitem{isar2024docs}
S. Leichsenring, ``Isar Database: Extremely Fast, Easy to Use, and Flexible NoSQL Database for Flutter,'' 2024. [Online]. Available: \url{https://isar.dev/}. [Accessed: 29-Jul-2026].

\bibitem{supabase2024docs}
Supabase Inc., ``Supabase Documentation: PostgreSQL Row Level Security and Stored Procedures,'' 2024. [Online]. Available: \url{https://supabase.com/docs}. [Accessed: 29-Jul-2026].
...
D. P. Kingma and J. Ba, ``Adam: A Method for Stochastic Optimization,'' in \textit{Proc. 3rd Int. Conf. Learn. Representations (ICLR 2015)}, San Diego, CA, USA, 2015, pp. 1--15.

\bibitem{srivastava2014dropout}
N. Srivastava, G. Hinton, A. Krizhevsky, I. Sutskever, and R. Salakhutdinov, ``Dropout: A Simple Way to Prevent Neural Networks from Overfitting,'' \textit{Journal of Machine Learning Research}, vol. 15, no. 56, pp. 1929--1958, 2014.
